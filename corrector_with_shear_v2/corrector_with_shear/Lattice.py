...
from corrector_with_shear.Pauli import Pauli
from corrector_with_shear.get_correction import get_correction

def nx_template(d, shear):
    # This just makes an nx graph with the right boundary conditions and weighted edges, for use in MWPM
    # Is prepped for changing shear in TWO dimensions
    G = nx.Graph()
    ...
    for row in range(d):
        G.add_edge((row, 0), (((row+shear)%d), d-1), weight=1)
    for col in range(d):
        G.add_edge((0, col), (d-1, ((col-shear)%d)), weight=1)
    for col in range(d-1):
        G.add_edge(((d-1), col), ((d-1), (col+1)%d))
    for row in range(d-1):
        G.add_edge((row, (d-1)), (((row+1)%d), d-1))  
...
class Lattice:
    ...
    def light_checks(self):
        ...
        if self.shear == 0:
            ...
            for row in range(self.d):
                for col in range(self.d):
                    if (row, col) == (0, 0):
                        val = (self.q[(row, col, 'h')] + self.q[(row, col, 'v')] + self.q[((row+self.shear)%self.d, self.d-1, 'h')] + self.q[(self.d-1, (col-self.shear)%self.d, 'v')])  %2
                        if val:
                            self.c[row, col] = 1
                        else:
                            self.c[row, col] = 0
                    elif (row == 0 and col != self.d-1):
                        val = (self.q[(row, col, 'h')] + self.q[(row, col, 'v')] + self.q[(row, (col-1)%self.d, 'h')] + self.q[((self.d-1), (col-self.shear)%self.d, 'v')]) %2
                        if val:
                            self.c[row, col] = 1
                        else:
                            self.c[row, col] = 0
                        ...
                        if val:
                            ...
                            self.c[row, col] = 0
                    elif (row != 0 and row != self.d-1 and col == 0):
                        val = (self.q[(row, col, 'h')] + self.q[(row, col, 'v')] + self.q[((row+self.shear)%self.d, self.d-1, 'h')] + self.q[((row-1)%self.d, col, 'v')] ) %2
                        if val:
                            self.c[row, col] = 1
                        else:
                            self.c[row, col] = 0
                    elif (row == self.d-1 and col == 0):
                        val = (self.q[(row, col, 'h')] + self.q[(row, col, 'v')] + self.q[((row+self.shear)%self.d, self.d-1, 'h')] + self.q[((row-1)%self.d, col, 'v')] ) %2
                        if val:
                            self.c[row, col] = 1
                        else:
                            self.c[row, col] = 0
                    elif (row == self.d-1 and col == 0 and col != self.d-1) or (row == self.d-1 and col == self.d-1) or (row != self.d-1 and row !=0 and col == self.d-1):
                        val = (self.q[(row, col, 'h')] + self.q[(row, col, 'v')] + self.q[(row, (col-1)%self.d, 'h')] + self.q[((row-1)%self.d, col, 'v')] ) %2
                        if val:
                            self.c[row, col] = 1
                        else:
    ...
    def correct(self, nx_template):
        ...
        qubit_addresses, paths = get_correction(self.matches, self.T, self.d, self.shear)
    ...
    def logical_operators(self):
        ...
        if self.shear == 0:
            ...
            vert=[(self.d*2*n)+(2*self.d-2) for n in range(self.d-(1))]
            vert.append(vert[-1]+1)
            for n in range(self.shear-1):
                vert.append(vert[-1]-(2))
            vert.append((self.d*2*(self.d-1))+(2*self.d-2)-((self.shear*2)))
            self.vert = vert
            horiz=[(2*n)+1 for n in range(self.d-1)]
            horiz.append(horiz[-1]+(2*self.d)-1)
            for n in range(self.shear-1):
                horiz.append(horiz[-1]+(2*self.d))
            horiz.append(horiz[-1]+3)
            self.horiz = horiz
            for i in range(self.n):
                if i in horiz:
                    hop += 'Z'
